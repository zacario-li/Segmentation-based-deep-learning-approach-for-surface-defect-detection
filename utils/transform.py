'''
transform copyed from pspnet's source code
...
class Crop(object):
    ...
    def __call__(self, image, label):
        ...
        if pad_h > 0 or pad_w > 0:
            ...
            image = cv2.copyMakeBorder(image, pad_h_half, pad_h - pad_h_half, pad_w_half, pad_w - pad_w_half, cv2.BORDER_CONSTANT, value=self.padding)#图像增加边界
...
class RandRotate(object):
    ...
    def __call__(self, image, label):
        if random.random() < self.p:#这个是随机的翻转
...
class RandomGaussianBlur(object):
    ...
    def __call__(self, image, label):
        if random.random() < 0.5:  #(5, 5)表示高斯矩阵的长与宽都是5，标准差取0
...
#add by lizhijian for color jitter
class ColorJitter(object):
    def __init__(self, b=0.5, c=0.5, s=0.5, h=0.5):
        self.b = b
        self.c = c
        self.s = s
        self.h = h
    ...
    def __call__(self, image, label):
        ...
        image = TS.ColorJitter(self.b, self.c, self.s, self.h)(image)
        #convert PIL to cv2
